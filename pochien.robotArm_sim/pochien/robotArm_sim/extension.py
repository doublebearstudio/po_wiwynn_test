...
class MyExtension(omni.ext.IExt):
    ...
    def _on_update_task_params(self, task):
        ...
        if task['setup'] is not None:
            ...
            # Update scheduler's task if it exists
            if self._scheduler and task['name'] in self._scheduler.tasks:
                scheduler_task = self._scheduler.tasks[task['name']]
                scheduler_task.robot_position = new_robot_pos.copy()
                print(f"[RobotPickup] {task['name']} - Updated scheduler robot_position to {new_robot_pos}")
    ...
    def _on_setup_clicked(self):
        ...
        try:
            ...
            for task_idx, task in enumerate(self._tasks):
                ...
                # DEBUG: Print task configuration before creating setup
                print(f"[DEBUG] About to create setup for {task['name']}:")
                print(f"  task['robot_position'] = {task['robot_position']}")
                print(f"  task['initial_position'] = {task['initial_position']}")
                print(f"  task['target_position'] = {task['target_position']}")
